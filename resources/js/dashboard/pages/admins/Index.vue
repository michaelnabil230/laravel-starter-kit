...
const {
    ...
    confirmDeleteAll,
    resetItemSelection,
} = useItemSelectionManager(props.admins.data, route('dashboard.admins.pluck-destroy'), {
    onSuccess: () => {
...
const { openDelete, confirmDelete, openDeleteModal, closeDeleteModal } = useDeletion<App.Models.Admin>(
...
const filter = useFilter(route('dashboard.admins.index'), props.filters);
...
<template>
    <AppLayout :title="__('resources.admin.plural')" :breadcrumbs="breadcrumbs">
        ...
        <Delete v-model="openDelete" resource="admin" @close="closeDeleteModal" @confirm="confirmDelete" />
