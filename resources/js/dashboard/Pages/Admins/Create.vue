...
const submit = () => {
    form.post(route('dashboard.admins.store'), {
        ...
        preserveState: true,
        onSuccess: () => {
            dashboardApp.success(__('global.actions.created', { resource: __('resources.admin.singular') }));
            form.reset();
        },
    ...
    >
        ...
        >
            ...
            <div class="space-y-4 p-5">
                ...
                <div v-if="$page.props.auth.can.owner">
                    <InputLabel :value="__('resources.branch.plural')" for="branches_id" class="mb-2" />

                    <Select
                        :attribute="__('resources.branch.plural')"
                        v-model="form.branches_id"
                        :endPoint="route('api.dashboard.branches')"
                        :hasError="form.errors.hasOwnProperty('branches_id')"
                        id="branches_id"
                        multiple
                    />

                    <InputError :message="form.errors.branches_id" />
                </div>
            </div>
            <!-- End Body -->
